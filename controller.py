...
class Controller():
    ...
    def control(self, segmented_image, yolo_output):
        # Reset if counter exceeds threshold
    ...
    def handle_areas(self, areas, segmented_image):
        ...
        if areas > 550.0 and self.majority_class == 'turn_left':
            # Check if at junction 3 or 4 (based on corner sums)
            is_junction_3_or_4 = self.sum_top_corner > 18_000 or (self.sum_top_corner <= 18_000 and (self.sum_left_corner <= 2_000 or self.sum_top_corner >= 17_500))
            if not is_junction_3_or_4:
